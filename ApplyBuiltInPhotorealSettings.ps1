# ApplyBuiltInPhotorealSettings.ps1
# - Creates Assets/Editor/ApplyBuiltInPhotorealSettings.cs
# - Runs Unity in batchmode to apply Linear/Deferred, GI, probe helper, and drops a verification file.

$ErrorActionPreference = 'Stop'
$proj = Get-Location
$edDir = Join-Path $proj 'Assets/Editor'
$newDirs = @(
  $edDir,
  (Join-Path $proj 'Assets/Trevi/Validation')
)
$newDirs | ForEach-Object { if (!(Test-Path $_)) { New-Item -ItemType Directory -Force -Path $_ | Out-Null } }
...
$outCs = Join-Path $edDir 'ApplyBuiltInPhotorealSettings.cs'
Set-Content -LiteralPath $outCs -Encoding UTF8 -Value $cs

# Try common Unity 5 path; allow override via $env:UNITY_EXE
...
if ([string]::IsNullOrWhiteSpace($unityExe) -or !(Test-Path $unityExe)) {
  Write-Error "Unity.exe not found. Set `$env:UNITY_EXE to your Unity 5.3.4f1 Editor path."
}

# Run Unity headless to apply
& $unityExe -batchmode -quit -projectPath $proj -executeMethod ApplyBuiltInPhotorealSettings.Run | Out-Null

# Minimal test: verify marker file
$marker = Join-Path $proj 'Assets\Trevi\Validation\SETTINGS_APPLIED.txt'
if (!(Test-Path $marker)) { Write-Error "Apply step failed (marker not found): $marker" }
Write-Host "OK: Built-in photoreal defaults applied. Marker: $marker"
